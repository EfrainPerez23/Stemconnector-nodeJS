...
let internModel = {
    ...
    addIntern: function(ev, callback) {
        return db.query('INSERT INTO `Intern` VALUES (?,?,?,?,?,?,?,?)', [null, ev.name, ev.country, ev.photo, ev.flagImage, ev.description, ev.rol, ev.linkedIn], callback);
    },
    ...
    updateIntern: function(id, ev, callback) {
        return db.query('UPDATE  `Intern` SET  `name`= ?, `country`= ?, `photo`= ?, `flagImage`= ?, `description`= ?, `rol`= ?, `linkedIn` =?  WHERE  `Intern`.`idIntern` =  ' + id + ';', [ev.name, ev.country, ev.photo, ev.flagImage, ev.description, ev.rol, ev.linkedIn], callback);
    }
};
