...
let internControl = require('../Models/InternModel');

router.get('/:id?', (req, res) => {
    const id = db.escape(req.params.id);
    if (req.params.id) {
        internControl.getIntern(id, function(err, result) {
            // Server err
            if (err) {
                res.status(500);
        ...
        internControl.getInterns(function(err, result) {
            //server err
            if (err) {
                res.status(500);
...
router.post('/', function(req, res) {
    internControl.addIntern(req.body, function(err, result) {
        if (err) {
            db.on('error', (dbErr) => {
...
router.delete('/:id', function(req, res) {
    internControl.deleteIntern(db.escape(req.params.id), function(err, result) {
        if (err) {
            db.on('error', (dbErr) => {
                console.log('[mysql error]', dbErr);
            });
...
router.put('/:id', function(req, res) {
    internControl.updateIntern(db.escape(req.params.id), req.body, function(err, result) {
        if (err) {
            db.on('error', (dbErr) => {
                console.log('[mysql error]', dbErr);
            });
